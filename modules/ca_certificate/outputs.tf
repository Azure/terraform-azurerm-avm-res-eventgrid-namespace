output "description" {
  ...
  value       = try(azapi_resource.ca_certificate.output.properties.description, null)
}

output "encoded_certificate" {
  description = "The base64 encoded certificate (PEM or DER format)."
  sensitive   = true
  value       = try(azapi_resource.ca_certificate.output.properties.encodedCertificate, null)
}
